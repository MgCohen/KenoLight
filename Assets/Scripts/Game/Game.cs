...
public class Game : MonoBehaviour
{
    ...
    {
        currentSorteio = _sorteio;
        Draw(0);
    }

    Sorteio currentSorteio;
    int currentNumber;
    int index;
    ...
    public void Draw(int i)
    {
        next = false;
        index = i;
        currentNumber = currentSorteio.balls[i];
        control.Draw(currentNumber, (i == currentSorteio.balls.Count - 1));
    }

    public void SetTable()
    {
        table.SetNumber(currentNumber);
        usedBalls.Add(currentNumber);
        table.SetCount(usedBalls.Count);
        Sort(usedBalls.Count - 1, currentSorteio);
        //proximo draw
        if (index == currentSorteio.balls.Count - 1) Clear();
        else next = true;
    }
